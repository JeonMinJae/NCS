...
import mj.project.clonemusic.databinding.FragmentPlayerBinding
import mj.project.clonemusic.service.*
...
class PlayerFragment : Fragment(R.layout.fragment_player) {
        ...
        binding?.let { binding ->
            player?.addListener(object: Player.Listener {
                override fun onIsPlayingChanged(isPlaying: Boolean) {
                    ...
                    if (isPlaying) {
                        binding.playControlImageView.setImageResource(R.drawable.pause)
                    } else {
                        binding.playControlImageView.setImageResource(R.drawable.play_arrow)
                    }
                }
    ...
    private fun initPlayListButton(fragmentPlayerBinding: FragmentPlayerBinding) {
        fragmentPlayerBinding.playlistImageView.setOnClickListener {
            ...
            model.isWatchingPlayListView = !model.isWatchingPlayListView // !true = false
        }
    }
    ...
    private fun updateSeek() {
        ...
        view?.removeCallbacks(updateSeekRunnable) // ui변경이 이루어진후 callback을 제거해준다.
        // idle : 재생실패, ended: 재생마침
    ...
    private fun updateSeekUi(duration:Long, position: Long) {
        binding?.let { binding ->
            ...
            binding.playTimeTextView.text = String.format("%02d:%02d",
                TimeUnit.MINUTES.convert(position, TimeUnit.MILLISECONDS), //millsecond를 분으로 바꿈
                (position / 1000) % 60)
